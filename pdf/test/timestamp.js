 ...
 *
 * ECDSA P-256 Sertifikaları ve katı ASN.1 DER Kuralları (RFC 3161, RFC 5035, RFC 5758)
...
const CFG = {
  ...
  tsaKeyPath: "./certs/timestamp.key",
  tsaCertPath: "./certs/timestamp.crt",

  tsaPolicyOID: "1.3.6.1.4.1.65133.1.4",
...
function parseTimeStampReq(der) {
  ...
  // İstemcinin gönderdiği messageImprint'in orijinal byte'larını alıyoruz
  const rawMessageImprint = der.slice(tlv.valueOff - tlv.headerLen, tlv.end);
  ...
  return { hashAlgOID, hashedMessage, reqPolicyOID, nonceBytes, certReq, rawMessageImprint };
}
...
function buildTSTInfo({ policyOID, rawMessageImprint, serialBytes, nonceBytes }) {
  ...
  // messageImprint'i doğrudan istemciden geldiği gibi yerleştiriyoruz
  const parts = [version, policy, rawMessageImprint, serialNumber, genTime];
...
// ECDSA standartları gereği algoritma tanımlanırken NULL parametre İÇEREMEZ.
...
function buildSigningCertificateV2(certDerLeaf) {
  ...
  // 1. OpenSSL'in sertifikayı doğrulayabilmesi için Issuer ve Serial bilgilerini çıkarıyoruz
  const { issuerTLV, serialBytes } = parseCertIssuerAndSerial(certDerLeaf);
  
  // GeneralNames ::= SEQUENCE { [4] EXPLICIT Name }
  const directoryName = derExplicit(4, issuerTLV);
  const generalNames = derSeq(directoryName);
  const issuerSerial = derSeq(generalNames, derIntFromUnsignedBytes(serialBytes));

  // 2. SHA-256 algoritmasını parametresiz (NULL olmadan) tanımlıyoruz (RFC 5754 Kuralı)
  const hashAlgId = algIdNoParams(OID.sha256);

  // 3. ESSCertIDv2'yi eksiksiz bir şekilde (Hash Alg, Hash Değeri, Issuer+Serial) oluşturuyoruz
...
function buildSignerInfo({ certDerLeaf, signedAttrsSetDER }) {
  ...
  // RFC 5754: SHA256 özet algoritması NULL parametre İÇEREMEZ
  const digestAlg = algIdNoParams(OID.sha256);
  ...
  // İmza Algoritması olarak ECDSA kullanıyoruz ve NULL parametresi eklemiyoruz
  const sigAlg = algIdNoParams(OID.ecdsaWithSHA256);

  // EC Key ile doğrudan imzalıyoruz
...
function buildSignedData({ eContentTypeOID, eContentDER, certsDerList, signerInfoDER }) {
  ...
  // RFC 5754: SHA256 özet algoritması listesinde NULL parametre İÇEREMEZ
  const digestAlgorithms = derSetOfSorted([algIdNoParams(OID.sha256)]);
  ...
  const server = http.createServer(async (req, res) => {
    try {
      if (req.method === "GET" && req.url === "/") {
        ...
        res.end(`
Service Main Security Computer
